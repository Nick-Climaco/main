...
def exercise02(a):
    # The function exercise02() receives a Python list and converts it to an ndarray. Convert the list to a numpy
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    s = pd.Series(np.random.randint(0, n, size=n), index=pd.date_range(start="1/1/2010", periods=n))
    s.cumsum().plot()
    # ------ Place code above here /\ /\ /\ ------
    return s
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    series_1 = pd.Series(words)
    df = series_1.map(lambda word: len(word))
    # ------ Place code above here /\ /\ /\ ------
    return df
...
def exercise15():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    return_values = exercise08()
    df_1 = return_values[0]
    df = df_1.iloc[::5, [0, 5]]
    # ------ Place code above here /\ /\ /\ ------
    return df
