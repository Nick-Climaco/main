...
A streak of length 1 means that the most recent shot was a hit(H) thus beginning a streak. 

A streak of length 0 means that the most recent shot was a miss(M) thus does not start a streak.
...
According to the graph provided, his typical streak length was of lenght 1 meaning that after he made a shot he usually misses the next one. His longest streak of baskets was 4 shots he made in a row. 
...
In the simulation above, there are 39 heads and 61 tails. 
...
```{r set-seed}
set.seed(12041998)                 # make sure to change the seed
```
...
Without setting a seed, the graph would look "somewhat similar" with the streak of length 0 would be the most frequent since P(M) > P(H), so it is more likely a single experiment of a shot would miss, but there is a slight difference between simulation due to randomness. 
...
Comparing the two graphs does show similarities, it does not provide conclusive evidence that Kobe had "hot hand". This would require further analysis in order to conclude whether this phenomenon is real. Also, we need to collect more data since the sample size is relatively small. A small sample size would hinder the reliability of the conclusions we can draw from the data. 
