...
#### Response to Exercise 1
    - Visually the three graphs show different binwidths which can obscure data with lower counts because if the binwidth is large then it will take up the space of
      the other data.
...
```
    - There are 68 flights that met the criteria.
...
```
    - Based on the graph, we can observe that flights to San Francisco arrives 4 minutes early on average. Also, we observe that the distribution is skewed to the left indicating that flights usually arrived earlier than expected.
...
```
Carriers DL and AU have the widest range implying more variability.
...
#### Response to q5
Choosing the month with lowest mean departure delay would lower the chance of encountering a delay, but mean that does not mean there won't be any delays since the mean can be inlfuenced with outliers. On the other hand, choosing the month with the lowest median departure delay picks the middle number in the data and outliers do not skew the median, but it does not appropriately represent how the data is distributed.
...
#### Response to q6
Based on the data. LGA has the best departure time percentage of 72.8%. So, I would choose LGA to fly out of. 
...
#### Response to q7 
```{r}
# distance is in hours while air_time is in minutes
...
nycflights2 %>%
    ...
    labs(x= "Distance from origin to destination", 
         y="Average of Speed of the aircraft (in miles)", 
         title = "Average Speed vs Distance of a flight" ) +
...
```{r}
nycflights %>%
    filter(arr_delay < 10 & arr_delay > -10) %>%
    ggplot(aes(x = dep_delay, y = arr_delay, color = carrier)) +
    geom_point()
```
 Based the graph, we can roughly suggest the cuttoff point for departure delay is around 30 minutes delayed and still expect to arrive on time (on time on this case is within 10 minutes of the planned arrival time).
