...
We would not expect another student's sample proportion to be identical because every time we take a sample from the population there will be a difference to some degree due to randomness.
...
95% Confidence interval means that you the true value of the proportion in within the 95% area of the normal distribution. i,e, that the true value is within 1.96 standard deviations away from the mean. we get 
the 95 from P(-1.96 < Z < 1.96) = 0.95
...
Using the Z value 1.96 we would be 95% confident that the true population proportion would be captured in the confidence interval. Would my neighbor's interval capture this value? We would not know it is possible 
for my neighbor's interval and my interval to not overlap and it would depend on his sample size and my sample size. Also, we may be using different z scores 1.96, 1.65, or 2.58 for our calculations which cause 
different intervals.
...
It would depend on what each student used for confidence intervals. A z score of 1.96 would have a wider interval than a z score of 1.65 which has has a narrower interval and less likely to capture 
...
We observe that 2 out of the 50 confidence intervals does not capture the true population proportion which agrees with the 95% confidence interval. 48/50 = 0.96
...
The selection of the confidence interval affects the width of the interval. A higher confidence interval would the widen the area under the curve. A lower interval would decrease the area under the curve.
...
We can state that we are 99% confidence that the true proportion of US Adults who think climate change does affect them is between [0.43, 0.77].
...
Using the shiny app, we set the confidence interval to be 0.99 and the output graph showed that all 50 confidence intervals capture the true proportion of the people who said yes.
...
Now, we use a 0.90 confidence interval we expect the interval to narrow and some intervals to not capture the true proportion. In the app, 3 out of 50 did not capture the population mean.
```{r}
samp |> 
    specify(response = climate_change_affects, success = "Yes") |> 
    generate(reps = 1000, type = "bootstrap") |> 
...
As the confidence level decreases, then the confidence interval also decreases. 
As the confidence level increases, then the confidence interval also increases.
...
We observe that changing the number of bootstrap samples, the intervals fluctuates a bit. It starts to change when having less than 1000 bootstrap samples.
We tried using 100, 500, 1000, 5000 for the number of bootstraps samples. 
