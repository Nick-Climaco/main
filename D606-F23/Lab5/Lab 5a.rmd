...
After taking a sample of 50, we observe that the people who state that they benefits from the work of scientists have an overwhelming majority with 88% of the sample.
...
We would expect our sample proportion to not necessarily be the same because due to the randomness in sampling the population. There will be some degree of difference between our two sample proportion. 
...
After taking samples of sizes: 50, 100 and 1000, we observe that as n increases and approaches the population size, p_hat also approaches p as a corollary of the law of large numbers.
...
There are 15 000 elements in sample_props50 since its take 15 000 samples of size 50 from the population. The sampling distribution we see above looks to follow a normal distribution and notice the "center" of the graph is 0.2 which is the P(scientist_work = "doesnt benefit") in the population. 
```{r}
ggplot(data = sample_props50, aes(x = p_hat)) +
  geom_histogram(binwidth = 0.02) +
  labs(
...
The mean of the sampling distribution is around the 0.2 and as the sample size increase the standard error decreases. The shapes of each sample of different size were similar to a bell-shaped curve. Changing these values will approach the population value as we keep increasing the number of simulation but overall the graphs looked similar.
...
```{r}
samp_15_benefit <- global_monitor |> 
    sample_n(15) |> 
    count(scientist_work) |> 
    mutate(p_hat = n / sum(n)) |> 
    filter(scientist_work == "Benefits")
...
With the graph below, we would guess that the true proportion that think the work of scientist benefit their lives is 80% or 0.8
...
The sample follows a normal distribution, we would guess that the true proportion for benefits in 80%.
```{r}
...
If we were to estimate the true value, we would assume that the we would prefer a sampling distribution with a smaller spread since it have a smaller variance.
```{r}
samp2 |> 
    count(scientist_work) |> 
    mutate(p_hat = n / sum(n)) |> 
