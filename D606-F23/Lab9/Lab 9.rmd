...
This appears to be observational study since the data its using a collected from actual students ratings for professors for the class and their physical appearance. Thus, there
will be some bias for individual students preferences. A possible way to rephrase the question is "Is there correlation between the physical appearance and course evaluations?"
...
```{r}
ggplot(data = evals, aes(x = cls_students, y = cls_did_eval)) +
           geom_point()
```
...
With geom_jitter(), we can better see the density and distribution of the data.
...
The beta coefficient of 0.067 of avg beauty means that, on average, the predicted score increases 0.067 per point in beauty. 

With a p-value less than 0.05, suggests that average beauty is a significant predictor.

But the R squared values indicate that this model is a not a fit since it only explains 3% of the variability of the data.
...
```{r}
lm_model <- lm(score ~ bty_avg, data = evals)
...
It seems that most of the residuals are between -1 and 1 which implies that the residuals do not greatly devaite from the line.
...
bty_avg is still a significant predictor of score. The addition of gender to the model descrease the coefficient for bty_avg and lowered the R-squared value as well
...
For two professors who received the same beauty rating white&black professors tends to score higher than color.
...
The variable with highest p value is cls_prof because the number of professors teaching would not impact model since we are taking look at individual professor rating.
...
```{r}
sus_lm <- lm(score ~ cls_profs, data = evals)
summary(sus_lm)
```


We see that cls_profs is not a significant predictor of score and very low R squared indicates that the model is not a good fit. In addition, a small F statistic and high p value further indicates that this variable is not significant


13. Interpret the coefficient associated with the ethnicity variable.


The coefficient of ethnictiy in the model is 0.123 with p-value of 0.1169 which suggests that this varaible is nto a significable indicator for score
...
```{r}
another_lm <- lm(score ~ rank + gender + ethnicity + language + age + cls_perc_eval 
...
```{r}
best_lm <- lm(score ~  gender +  language + age + cls_perc_eval 
             + cls_credits + bty_avg 
...
```{r}
qqnorm(best_lm$residuals)
qqline(best_lm$residuals)
```
...
It might have an impact on the regression becuase some courses will be taught by the same professors which may introduce bias in the analysis.
...
The most significant characteritics of a professor to receive a high evaluation score are if they are black&white english speaking male teaching a multi credit course

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?


It would be not be right to generalize conculsion infered from a sample from one universtiy and apply it to any universities without first taking samples from every other university. So no, I would not be comfortable generalizing this conclusions.
