...
```
The correlation between pf_expression_control and pf_score 0.796 which indicative a strong positive correlation.
...
The smallest sum of squares is 952.153 after running the code repeatedly.
...
The Least Square Method gives a linear function:
$$
f_{y}(x_i) = -6.29 + 1.65x_i
$$
where pf_expression_control is the predictor variable.
```{r}
model2 <- lm(pf_expression_control ~ hf_score, data = hfi)
summary(model2)
```
...
Just looking at the regression line, we want to know what a 6.7 rating on the pf_expression_control would predict.
We know from the equation, 

\[

f_y(6.7)  & = 4.62 + 0.49(6.7) \\
& = 7.90
...
ggplot(data = m1, aes(x = .fitted, y = .resid)) +
  ...
  ylab("Residuals")
```
Most residuals are between 2 points away from the regression line which suggests that the residuals do not greatly deviate from the line. This indicate the positive correlation between the two variables.
* * *
</br>
...
ggplot(data = m1, aes(sample = .resid)) +
  stat_qq()
```
stat_qq shows us the the sample closely follows normal which satisfies the condition of normality. 
* * *
</br>
...
We saw on the residual graph that the variability is constant.
* * *
...
At a glance, the is a positive correlation between pf_association as a the predictor and pf_religion. Which implies that higher pf_association would in turn have a higher pf_religion.

```{r}
hfi |> 
    ggplot(aes(x = pf_association, y = pf_religion)) +
    geom_point() 
```
...
We can conclude that hf_score in model2 explains a greater proportion of the variance in pf_expression_control than pf_religion in model3.

```{r}
model3 <- lm(pf_association ~ pf_religion, data = hfi)
summary(model3)
```
